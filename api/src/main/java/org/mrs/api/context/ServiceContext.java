...
import org.mrs.messagesource.impl.DefaultMessageSourceServiceImpl;
...
public class ServiceContext implements ApplicationContextAware {
	 ...
	 * false so the mrs classloader is used instead
	 */
	...
	/**
	 * There should only be one ServiceContext per mrs (java virtual machine). This method
	 ...
	 * refreshing (being added/removed) and/or mrs is shutting down
	 */
	...
	public void setModuleService(List<Object> params) {
		...
		// load the given 'classString' class from either the mrs class
		// loader or the system class loader depending on if we're in a testing
		// environment or not (system == testing, mrs == normal)
		try {
			if (!useSystemClassLoader) {
				...
				try {
					...
					if (log.isDebugEnabled()) {
						...
						//cls2 is the mrs class loader, like above.
						log.debug("cls==cls2: "
	 ...
	 * <code>useSystemClassLoader</code> is set to true. If false, the mrs class loader is used
