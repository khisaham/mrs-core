...
import org.mrs.api.handler.ExistingVisitAssignmentHandler;
import org.mrs.customdatatype.datatype.BooleanDatatype;
import org.mrs.customdatatype.datatype.FreeTextDatatype;
...
import org.mrs.scheduler.SchedulerConstants;
...
public final class OpenmrsConstants {
	...
	/**
	 * hack alert: During an ant build, the mrs api jar manifest file is loaded with these
	...
	/**
	 * This holds the current mrs code version. This version is a string containing spaces and
	 * words.<br>
	 * The format is:<br>
	...
	/**
	 * This holds the current mrs code version in a short space-less string.<br>
	 * The format is:<br>
	...
	public static String getOpenmrsProperty(String property) {
		...
		if (file == null) {
			log.error("Unable to find the mrs.properties file");
			return null;
		}
		...
		try {
			...
			log.error("Unable to parse the mrs.properties file", e);
		}
		finally {
	...
	public static String DATABASE_NAME = "mrs";
	
	public static String DATABASE_BUSINESS_NAME = "mrs";
	...
	/**
	 * The name of the runtime property that a user can set that will specify where mrs's
	...
	public static final String DEFAULT_ADDRESS_TEMPLATE = "<org.mrs.layout.address.AddressTemplate>\n"
	        ...
	        + "   <requiredElements>\\n\" + \" </requiredElements>\\n\" + \" </org.mrs.layout.address.AddressTemplate>";
	...
	public static final List<GlobalProperty> CORE_GLOBAL_PROPERTIES() {
		...
		props.add(new GlobalProperty("mail.from", "info@mrs.org", "Email address to use as the default from address"));
		...
		props
		        .add(new GlobalProperty(
		                ...
		                "org.mrs.api:" + LOG_LEVEL_INFO,
		                "Logging levels for log4j.xml. Valid format is class:level,class:level. If class not specified, 'org.mrs.api' presumed. Valid levels are trace, debug, info, warn, error or fatal"));
		
		props.add(new GlobalProperty(GP_LOG_LOCATION, "",
		        "A directory where the OpenMRS log file appender is stored. The log file name is 'mrs.log'."));
		...
		props.add(new GlobalProperty(GLOBAL_PROPERTY_REPORT_BUG_URL, "http://errors.openmrs.org/scrap",
		        "The mrs url where to submit bug reports"));
	...
	/**
	 * It points to a directory where 'mrs.log' is stored.
	 * 
	 * @since 1.9.2
	 */
	...
	public static final String LOG_CLASS_DEFAULT = "org.mrs.api";
